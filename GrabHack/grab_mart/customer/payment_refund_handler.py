...
class PaymentRefundHandler:
    """Customer-focused payment and refund management for grocery delivery with strict workflows"""
    ...
    def __init__(self, groq_api_key: str = None):
        ...
        try:
            ...
            logger.info("AI engine initialized successfully for payment refund handler")
    ...
    def handle_failed_payment_money_deducted(self, query: str, image_data: Optional[str] = None, username: str = "anonymous") -> str:
        ...
        if not payment_details["transaction_reference"] and not payment_details["order_attempt_info"]:
            return "📋 To resolve this payment issue, please provide your transaction reference number or details about when you attempted to place the grocery order."
    ...
    def handle_cod_marked_prepaid(self, query: str, image_data: Optional[str] = None, username: str = "anonymous") -> str:
        ...
        if not cod_details["order_id"] or not cod_details["expected_payment_method"]:
            return "📋 Please provide your grocery order ID and confirm what payment method you selected (COD or prepaid) when placing the order."
    ...
    def handle_refund_not_initiated(self, query: str, image_data: Optional[str] = None, username: str = "anonymous") -> str:
        ...
        if not refund_details["original_issue"] or not refund_details["promised_timeframe"]:
            return "📋 To track your refund status, please provide details about the original issue (missing/damaged grocery items) and when the refund was promised."
    ...
    def extract_double_charge_details(self, query: str) -> dict:
        ...
        if not self.ai_engine:
            # Fallback extraction
            details = {"order_id": "", "charge_amount": "", "charge_count": "2", "payment_method": "unknown", "charge_dates": []}
            if "order" in query.lower():
                details["order_id"] = "extracted_from_query"
            if "$" in query or "amount" in query.lower():
                details["charge_amount"] = "amount_mentioned"
            return details
    ...
    def verify_payment_charges(self, charge_details: dict, username: str) -> dict:
        ...
        # Simulate payment verification for grocery orders
    ...
    def generate_double_charge_response(self, decision: str, charge_details: dict, query: str) -> str:
        ...
        order_id = charge_details.get("order_id", "your grocery order")
        amount = charge_details.get("charge_amount", "the amount")

        if decision == "IMMEDIATE_FULL_REFUND":
            return f"""💰 **URGENT: Grocery Order Double Charge Refund**

We've confirmed that you were incorrectly charged twice for {order_id}.

**✅ Resolution:**
💵 **IMMEDIATE REFUND** of ${charge_details.get('duplicate_amount', 0):.2f} processing now
...
This is a serious payment error and we sincerely apologize for the inconvenience."""
            ...
            return f"""💳 **Grocery Order Double Charge Refund Confirmed**
...
Thank you for bringing this to our attention."""
            ...
            return f"""🔍 **Grocery Double Charge Under Review**
...
Upload these documents and we'll expedite your refund within 24 hours."""
    ...
    def extract_failed_payment_details(self, query: str) -> dict:
        """Extract failed payment details from complaint"""
        if not self.ai_engine:
            # Fallback extraction
            details = {"transaction_reference": "", "payment_method": "unknown", "deducted_amount": "", "attempt_time": "", "error_message": "", "order_attempt_info": ""}
            if "transaction" in query.lower() or "reference" in query.lower():
                details["transaction_reference"] = "mentioned_in_query"
            if "failed" in query.lower() or "error" in query.lower():
                details["order_attempt_info"] = "payment_failure_mentioned"
            return details
    ...
    def generate_failed_payment_response(self, decision: str, payment_details: dict, query: str) -> str:
        ...
        if decision == "IMMEDIATE_PRIORITY_REFUND":
            return f"""🚨 **PRIORITY: Grocery Order Failed Payment Refund**

We've confirmed your payment of {amount} was deducted despite grocery order failure.
...
This should never happen - we sincerely apologize for this system error."""
            ...
            return f"""💳 **Grocery Order Failed Payment Refund Confirmed**

Your payment of {amount} via {payment_method} was processed but grocery order failed to complete.
...
Thank you for reporting this payment system issue."""
            ...
            return f"""🔍 **Grocery Payment Failure Refund - Verification Complete**
...
Reference ID: SPC-{hash(query) % 10000:04d}"""
    ...
    def extract_cod_payment_discrepancy(self, query: str) -> dict:
        ...
        if not self.ai_engine:
            # Fallback extraction
            details = {"order_id": "", "expected_payment_method": "", "system_showing": "", "delivery_demand": "", "complaint_type": ""}
            if "cod" in query.lower():
                details["expected_payment_method"] = "cod"
            if "prepaid" in query.lower():
                details["system_showing"] = "prepaid"
            if "order" in query.lower():
                details["order_id"] = "mentioned_in_query"
            return details
    ...
    def generate_cod_prepaid_response(self, decision: str, cod_details: dict, query: str) -> str:
        ...
        order_id = cod_details.get("order_id", "your grocery order")
        expected_method = cod_details.get("expected_payment_method", "COD")

        if decision == "IMMEDIATE_SYSTEM_CORRECTION":
            return f"""🔧 **URGENT: Grocery Order System Error Correction**
...
This error should never have occurred. We sincerely apologize for the confusion."""
            ...
            return f"""✅ **Grocery Order Payment Method Issue Resolved**
...
Reference ID: PMI-{hash(query) % 10000:04d}"""
    ...
    def extract_refund_promise_details(self, query: str) -> dict:
        ...
        if not self.ai_engine:
            # Fallback extraction
            details = {"original_issue": "", "promised_date": "", "promised_timeframe": "", "promised_by": "", "elapsed_time": "", "reference_numbers": []}
            if "refund" in query.lower():
                details["original_issue"] = "refund_mentioned"
            if "promise" in query.lower() or "told" in query.lower():
                details["promised_by"] = "support_agent"
            return details
    ...
    def generate_refund_not_initiated_response(self, decision: str, refund_details: dict, query: str) -> str:
        ...
        original_issue = refund_details.get("original_issue", "your reported grocery issue")
        promised_timeframe = refund_details.get("promised_timeframe", "the promised timeframe")

        if decision == "IMMEDIATE_EXPEDITED_REFUND":
            return f"""🚨 **URGENT: Expedited Grocery Refund Processing**
...
This delay is unacceptable and we're taking immediate corrective action."""
            ...
            return f"""⚡ **Priority Grocery Refund Processing**
...
We apologize for not meeting our promised timeline."""
            ...
            return f"""🔄 **Accelerated Grocery Refund Processing**
...
We're fast-tracking the verification process for your grocery refund:
...
Your delayed grocery refund case requires specialized handling:
