...
class PackageCondition(Enum):
    ...
    SPILLED_CONTENTS = "item_contents_spilled"
    DAMAGED_CONTAINER = "container_physically_damaged"
    TEMPERATURE_COMPROMISED = "item_temperature_unsafe"
...
class OperationalHandler:
    def __init__(self):
        ...
        self.item_safety_time_limit = 30  # minutes
    ...
    async def _handle_package_tampered(self, context: OperationalContext) -> Dict[str, Any]:
        """Handle package tampering and item safety issues"""
        
        # Analyze package condition and safety
        package_analysis = await self._analyze_package_condition(context)
        
        # Item safety assessment
        safety_assessment = await self._assess_item_safety(context, package_analysis)
        ...
        # Store notification
        store_notification = await self._notify_store_package_issue(context, package_analysis)
    ...
    async def _analyze_package_condition(self, context: OperationalContext) -> Dict[str, Any]:
        ...
        ITEM_SAFETY_RISK: [LOW/MEDIUM/HIGH/CRITICAL]
        ...
        try:
            ...
            response = await self.groq_client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "You are an expert in item safety and package integrity assessment. Prioritize customer safety."},
    ...
    async def _handle_customer_unavailable(self, context: OperationalContext) -> Dict[str, Any]:
        ...
        item_quality_management = await self._manage_item_quality_during_wait(context, availability_analysis)
        ...
        return {
            ...
            "item_quality_management": item_quality_management,
            "performance_protection": performance_protection,
            "status": "handled",
            "timestamp": datetime.now().isoformat()
        }
    ...
    async def _handle_late_cancellation(self, context: OperationalContext) -> Dict[str, Any]:
        ...
        item_disposal = await self._manage_item_disposal(context, cancellation_analysis)
    ...
    async def _assess_item_safety(self, context: OperationalContext, package_analysis: Dict[str, Any]) -> Dict[str, Any]:
        """Assess item safety based on package condition"""
        
        safety_risk = package_analysis.get("ITEM_SAFETY_RISK", "MEDIUM")
        
        safety_assessment = {
            "risk_level": safety_risk,
            "safe_for_delivery": safety_risk in ["LOW", "MEDIUM"],
        ...
        if context.temperature_sensitive:
            ...
            if time_elapsed > self.item_safety_time_limit:
                safety_assessment["temperature_safety_compromised"] = True
                safety_assessment["safe_for_delivery"] = False
    ...
    async def _verify_order_mismatch(self, context: OperationalContext) -> Dict[str, Any]:
        ...
        Expected Items: {', '.join(context.items)}
        Order Value: ${context.order_value}
    ...
    async def _coordinate_replacement_order(self, context: OperationalContext, analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if analysis.get("REPLACEMENT_REQUIRED") == "true":
            ...
            coordination_result["store_can_remake"] = True  # Simulate store check
            
            if not coordination_result["store_can_remake"]:
                coordination_result["full_refund_processed"] = True
                coordination_result["alternative_store_suggested"] = True
        else:
            coordination_result = {
    ...
    async def _manage_item_disposal(self, context: OperationalContext, analysis: Dict[str, Any]) -> Dict[str, Any]:
        """Manage item disposal for cancelled orders"""
        ...
        disposal_management = {
            "disposal_method": "return_to_store" if time_since_pickup < 15 else "safe_disposal",
            "item_safety_compliant": True,
            "waste_minimization_attempted": True
        }
        
        if time_since_pickup < 15 and not context.temperature_sensitive:
            disposal_management.update({
                "return_to_store_feasible": True,
                "store_acceptance_likely": True
            })
        elif time_since_pickup < 30:
            disposal_management.update({
                "donation_to_charity_considered": True,
                "agent_compensation_item_value": min(context.order_value * 0.5, 12.0)
            })
        else:
            disposal_management.update({
                "safe_disposal_required": True,
                "item_safety_time_exceeded": True
            })
                ...
                messages=[
                    {
                        ...
                        "content": [
                            {
                                "type": "text",
                                "text": f"Analyze this package condition. Order contains: {', '.join(context.items)}. Assess: packaging integrity, item safety concerns, tampering evidence, spillage, container damage."
                            },
                            {
                                "type": "image_url",
                                "image_url": {
    ...
    def _create_fallback_package_analysis(self, context: OperationalContext) -> Dict[str, Any]:
        ...
        return {
            ...
            "ITEM_SAFETY_RISK": "MEDIUM",
            "DELIVERY_FEASIBLE": "false",
            "CUSTOMER_ACCEPTANCE_LIKELY": "false",
            "REPLACEMENT_REQUIRED": "true",
            "REFUND_AMOUNT": "full",
            "AGENT_LIABILITY": "NONE",
            "RECOMMENDED_ACTIONS": ["replacement_order", "customer_notification", "store_contact"]
        }
    ...
    def _check_order_system(self, order_id: str) -> str:
        ...
        try:
            ...
            cursor.execute('''
                ...
                FROM orders 
                WHERE id = ? AND service = 'grab_mart'
            ''', (order_id,))
            ...
            if result:
                order_id_db, start_loc, end_loc, store, customer_id, status, payment_method, details = result
                ...
                if status == 'completed' and order_id_db:
                    return "ONLINE_PAID - Payment confirmed in order system"
                else:
...
if __name__ == "__main__":
    async def test_operational_handler():
        ...
        package_context = OperationalContext(
            ...
            delivery_fee=3.50,
            payment_method="COD",
            items=["Groceries", "Vegetables", "Dairy"],
            ...
            package_condition_description="seal_broken_items_spilled"
        )
